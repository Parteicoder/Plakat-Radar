...
class LocalRepository(private val context: Context) {
    ...
    fun createLeaderTeam(teamName: String, leaderName: String): LocalTeamState {
        ...
        return current.copy(
            ...
            posters = emptyList(),
            events = listOf(PosterEvent(teamId = invite.teamId, posterId = "TEAM", actorDeviceId = current.deviceId, actorName = leader.displayName, action = "Team erstellt"))
        ).also(::save)
    }
    ...
    fun joinByInvite(invite: TeamInvite, memberName: String): LocalTeamState {
        ...
        return current.copy(
            ...
            posters = emptyList(),
            events = listOf(PosterEvent(teamId = invite.teamId, posterId = "TEAM", actorDeviceId = current.deviceId, actorName = member.displayName, action = "Per Teamleiter-QR beigetreten"))
        ).also(::save)
    }
    ...
    private fun eventsFromJson(arr: JSONArray) = (0 until arr.length()).map { arr.getJSONObject(it) }.map {
        PosterEvent(it.getString("id"), it.getString("posterId"), it.getString("teamId"), it.getString("actorDeviceId"), it.getString("actorName"), it.getString("action"), it.optLong("createdAt"))
    }
